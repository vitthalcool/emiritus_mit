...
{
	$con=mysqli_connect("108.174.147.161","miteme_program","k4jTui1SKXmplndM","miteme_programs") or die("cannot connect");
...
$source			=isset($_POST['utm_source'])? $xssClean->clean_input(trim(strip_tags($_POST['utm_source']))):'';
...
$url			=isset($_POST['url'])? $xssClean->clean_input(trim(strip_tags($_POST['url']))):'';
...
{
	header("Location: mastering-design-thinking.php");
	exit;
}
else
{
...
?>
<form class="form-horizontal mCustomScrollbar" role="form" id="frm" name="frm" action="https://www2.emeritus.org/l/134351/2018-01-30/3gck13" method="post">
...
<input type="hidden" id="first_name" name="first_name" value="<?php echo strip_tags($firstname);?>">				
<input type="hidden" id="last_name" name="last_name" value="<?php echo strip_tags($lastname);?>">		
<input type="hidden" id="email" name="email" value="<?php echo strip_tags($email);?>">
<input type="hidden" id="mobile" name="mobile" value="<?php echo strip_tags($phone);?>">
<input type="hidden" id="country" name="Country" value="<?php echo strip_tags($country);?>">
<input type="hidden" id="work_experience" name="work_experience" value="<?php echo html_entity_decode($workexp, ENT_NOQUOTES, 'UTF-8');?>">
<input type="hidden" id="utm_campaign" name="utm_campaign" value="<?php echo strip_tags($campaign);?>">
<input type="hidden" id="utm_content" name="utm_content" value="<?php echo strip_tags($content);?>">
<input type="hidden" id="utm_medium" name="utm_medium" value="<?php echo strip_tags($medium);?>">
<input type="hidden" id="utm_source" name="utm_source" value="<?php echo strip_tags($source);?>">
<input type="hidden" id="utm_term" name="utm_term" value="<?php echo strip_tags($term);?>">
<input type="hidden" id="email_verified" name="Email_verified" value="<?php echo strip_tags($email_verified);?>">
<input type="hidden" id="MIT_Email_Consent" name="MIT_Email_Consent" value="<?php echo strip_tags($MIT_Email_Consent);?>">
<input type="hidden" id="agree" name="agree" value="<?php echo strip_tags($agree);?>">
</form>
<script>
document.frm.submit();
</script>
